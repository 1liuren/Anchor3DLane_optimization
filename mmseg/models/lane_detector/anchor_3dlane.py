...
class Anchor3DLane(BaseModule):
    ...
    def __init__(self, 
        ...
        #         nn.Conv2d(backbone_dim//4, hidden_dim, 1),  # 修改输入通道数
        #         nn.Conv2d(backbone_dim//8, hidden_dim, 1),  # 修改输入通道数
        #     ]),
        #     'fpn_convs': nn.ModuleList([
        #         nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1),
        #         nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1),
        #     ]),
        #     'fusion_conv': nn.Conv2d(hidden_dim * 3, hidden_dim, 1)
        # })
    ...
    def build_fusion_module(self, feat_dims):
        """动态构建特征融合模块"""
        hidden_dim = feat_dims[0]
        ...
        self.feature_fusion = nn.ModuleDict({
            'lateral_convs': nn.ModuleList([
                ...
                ).to(device) for dim in feat_dims[1:]
            ]),
            'fpn_convs': nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1),
                    nn.BatchNorm2d(hidden_dim),
                    nn.ReLU(inplace=True)
                ).to(device) for _ in feat_dims[1:]
            ]),
        ...
        self.channel_attention = ChannelAttention(hidden_dim).to(device)
        self.spatial_attention = SpatialAttention().to(device)
    ...
    def feature_extractor(self, img, mask):
        ...
        # 选择主特征（最后一层）用于主处理
        feat = output[0] if self.neck else output[-1]
        feat = self.input_proj(feat)  # shape: [B, hidden_dim, H, W]

        # 初始化融合模块（只需一次）
        if self.feature_fusion is None:
            feat_dims = [feat.shape[1]]
            lateral_dims = [out.shape[1] for out in output[1:3] if out.shape[2:] == feat.shape[2:]]  # 空间对齐
            feat_dims += lateral_dims
            self.build_fusion_module(feat_dims)

        # 多尺度融合方式（改为加权求和）
        fused_feat = feat
        for i, feat_i in enumerate(output[1:3]):
            # 上采样到与 feat 尺寸一致
            upsampled_feat = F.interpolate(feat_i, size=feat.shape[2:], mode='bilinear', align_corners=True)

            # 1x1 conv + BN + ReLU
            lateral_feat = self.feature_fusion['lateral_convs'][i](upsampled_feat)

            # 3x3 conv + BN + ReLU
            lateral_feat = self.feature_fusion['fpn_convs'][i](lateral_feat)

            # 加权融合（与主特征相加）
            fused_feat = fused_feat + lateral_feat

        # 融合后的特征再做 attention
        fused_feat = self.feature_fusion['fusion_conv'](fused_feat)  # 只做 1x1 conv 不拼接
        
        # 添加注意力机制
        #feat = self.channel_attention(fused_feat)
